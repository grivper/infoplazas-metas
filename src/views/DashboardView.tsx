...
export const DashboardView: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Fila 3: Mesas + Rutas + KPAX (3 columnas iguales) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Card 4: Cumplimiento de Rutas */}
        {meta4 && (
          <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_12px_40px_rgba(44,47,48,0.06)] relative overflow-hidden hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <DonutChart percentage={meta4.progreso} color="green" />
            </div>
            {/* Barras de progreso por enlace */}
            <div className="space-y-2">
              {meta4.enlaces?.map((enlace) => {
                const pct = enlace.cumplimiento;
                const isCompletado = pct >= 100;
                
                return (
                  <div key={enlace.enlace} className="p-2 rounded-lg bg-surface-container-low">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-sm font-medium text-on-surface truncate">{enlace.enlace}</span>
                      <span className="text-xs font-bold" style={{ color: '#00694c' }}>
                        {enlace.visitadas}/{enlace.programadas} ({pct}%)
                      </span>
                    </div>
                    <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ 
                          width: `${Math.min(pct, 100)}%`, 
                          backgroundColor: isCompletado ? '#059669' : '#00694c'
                        }}
                      ></div>
                    </div>
                  </div>
